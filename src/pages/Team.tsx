    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-10" id="team-page">
        ...
        <div className="space-y-6 animate-fade-in">
          <div className="bg-teal-deep text-white p-8 sm:p-12 rounded-3xl grid grid-cols-1 md:grid-cols-4 gap-8 items-center border border-teal">
            <div className="flex justify-center md:col-span-1">
              <div className="w-28 h-28 sm:w-36 sm:h-36 rounded-full overflow-hidden border-4 border-white/20 shadow-lg bg-teal-pale flex items-center justify-center">
                {MEMBER_PHOTOS['pi'] ? (
                  <img
            ...
            <div className="space-y-4 md:col-span-3">
              <div className="space-y-1 text-center md:text-left">
                ...
                <h3 className="text-2xl sm:text-3xl font-serif font-bold text-white">{pi.name}</h3>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {currentStudents.filter(s => s.role.includes('MS')).map((student) => (
                <div key={student.id} className="bg-paper border border-line rounded-2xl p-7 flex gap-6 items-center hover:border-teal hover:shadow-sm transition-all">
                  <div className="w-20 h-20 rounded-full overflow-hidden bg-teal-pale shrink-0 border border-line flex items-center justify-center shadow-xs">
                    {MEMBER_PHOTOS[student.id] ? (
                      ...
                      <span className="font-bold font-serif text-2xl text-teal-deep select-none">
                        {student.photoInitials}
                      </span>
                    )}
                  </div>
                  <div>
                    <h4 className="font-serif font-bold text-teal-deep text-lg">{student.name}</h4>
                    <p className="text-xs sm:text-sm text-ink-soft font-medium">{student.role}</p>
                    <p className="text-[11px] sm:text-xs text-ink-faint font-mono mt-1">Session: {student.session} · GEB, RU</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      {activeSubTab === 'alumni' && (
        <div className="space-y-6 animate-fade-in" id="alumni-tab-panel">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {alumni.map((alum) => (
              <div key={alum.id} className="bg-paper border border-line rounded-2xl p-7 flex flex-col justify-between space-y-5 hover:border-teal hover:shadow-sm transition-all">
                <div className="flex gap-6 items-center">
                  ...
                  <div className="space-y-1.5 flex-1">
                    <div className="flex items-center justify-between gap-2 flex-wrap">
                      <span className="font-mono text-[10px] text-gold font-bold bg-gold-pale/35 px-2.5 py-0.5 rounded">
                        Session {alum.session}
                      </span>
                      <span className="text-[9px] uppercase font-bold tracking-wider font-mono text-ink-faint">GEB RU Alum</span>
                    </div>
                    <h4 className="font-serif font-bold text-teal-deep text-lg">{alum.name}</h4>
                    <p className="text-xs sm:text-sm text-ink-soft font-medium">{alum.role}</p>
                  </div>
                </div>

                {alum.nowPosition && (
                  <div className="bg-teal-pale/30 border border-teal/10 p-3 rounded-xl flex items-start gap-2 text-xs text-teal-deep leading-relaxed">
                    ...
                    <div>
                      ...
                      <p className="mt-0.5">{alum.nowPosition}</p>
                    </div>
                  </div>
                )}
              </div>
