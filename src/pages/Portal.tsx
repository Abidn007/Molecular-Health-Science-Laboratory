...
import { 
  ...
  Clock, Activity, Settings, UserCheck, Eye, EyeOff, CheckSquare
} from 'lucide-react';
...
export default function Portal({ user, onLogin, isLoggingIn, onDemoLogin, loginError, setLoginError, onCustomLogin }: PortalProps) {
  ...
  const handlePiLogin = (e: React.FormEvent) => {
    ...
    if (piEmailInput.trim().toLowerCase() === PI_EMAIL_ID) {
      const mockSession = {
        ...PI_MOCK_USER,
        isPi: true
      };
      ...
      setAuthError('Access denied. This email is not registered as Principal Investigator.');
    }
  };
  ...
  if (!user) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-12 space-y-12" id="login-gate">
        ...
        <div className="text-center space-y-4 max-w-2xl mx-auto">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs uppercase tracking-widest text-gold-pale font-mono font-extrabold">RU Workbench Intranet Portal</p>
          </div>
        ...
        {/* Dual Form grid for Students vs PI */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-stretch">
          
          {/* Card 1: Student Login Form */}
          <div className="bg-paper border border-line rounded-3xl p-8 shadow-sm flex flex-col justify-between hover:border-teal/30 transition-all">
            <div className="space-y-5">
              <div className="flex items-center gap-3 border-b border-line pb-4">
                ...
                <div>
                  <h3 className="font-serif font-bold text-lg text-teal-deep">Registered Student Access</h3>
            ...
            <div className="mt-6 pt-4 border-t border-line/60 flex items-center gap-2 text-[10px] text-ink-faint font-mono">
              <ShieldCheck className="w-4 h-4 text-teal" />
              <span>Session verified via local structural key rings</span>
            </div>
          </div>

          {/* Card 2: Lab PI Login Form */}
          <div className="bg-paper border border-line rounded-3xl p-8 shadow-sm flex flex-col justify-between hover:border-gold/30 transition-all bg-gradient-to-b from-paper to-gold-pale/10">
            <div className="space-y-5">
              <div className="flex items-center gap-3 border-b border-line pb-4">
                ...
                <div>
                  <h3 className="font-serif font-bold text-lg text-teal-deep">Principal Investigator</h3>
              ...
              <form onSubmit={handlePiLogin} className="space-y-4">
                <div className="space-y-1">
                  <label className="block text-[10px] font-mono text-ink-soft uppercase font-bold">PI Registered Email ID *</label>
                  <div className="relative">
                    <span className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-ink-faint">
                    ...
                    <input
                      ...
                      placeholder="e.g. khondokar@gmail.com"
                ...
                <div className="pt-2 text-center text-[11px] text-ink-soft leading-relaxed italic">
                  "Only the primary supervising professor’s academic email accounts are authorized to access this central directory module."
                </div>
            ...
            <div className="mt-6 pt-4 border-t border-line/60 flex items-center gap-2 text-[10px] text-ink-faint font-mono">
              ...
              <span>Bypasses standard LDAP with credential mapping</span>
            </div>
          </div>

        </div>

        {/* Support or fallback section */}
        <div className="bg-bg border border-line rounded-2xl p-5 text-center text-xs space-y-2 max-w-xl mx-auto">
          <p className="font-semibold text-ink-soft">Google Mail Account Integration</p>
          <p className="text-ink-faint leading-relaxed text-[11px]">
            If you need to sync raw DNA data directly into Firestore or connect Google Drive collections, you may also sign in via your registered institutional mail:
          </p>
          <div className="flex flex-wrap justify-center gap-3 pt-1">
            <button
              onClick={onLogin}
              disabled={isLoggingIn}
              className="inline-flex items-center gap-2 px-4 py-2 border border-line bg-paper rounded-xl text-xs font-semibold text-ink-soft hover:bg-bg transition-all cursor-pointer shadow-xs"
            >
            ...
            {onDemoLogin && (
              <button
                onClick={onDemoLogin}
                className="inline-flex items-center gap-1.5 px-4 py-2 bg-teal-pale text-teal-deep rounded-xl text-xs font-bold hover:bg-teal/15 transition-all cursor-pointer"
              >
      ...
      <div className="bg-teal-deep text-white p-6 sm:p-8 rounded-3xl border border-teal flex flex-col md:flex-row justify-between items-start md:items-center gap-6 shadow-md relative overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row md:flex-col lg:flex-row items-stretch sm:items-center gap-2.5 shrink-0 z-10 w-full md:w-auto">
          <div className="bg-white/10 backdrop-blur-md px-4 py-2 rounded-2xl text-xs text-white border border-white/15 space-y-0.5 flex flex-col justify-center">
            <span className="font-mono text-[9px] text-gold-pale font-bold uppercase tracking-wider">Access Node</span>
            <div className="flex items-center gap-1.5 font-mono text-[11px]">
              <ShieldCheck className="w-3.5 h-3.5 text-emerald-400" />
              <span>Rajshahi-GEB Secure VPN</span>
            </div>
          </div>
        </div>
      </div>
